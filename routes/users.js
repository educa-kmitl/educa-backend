...
router.get('/users', async (req, res) => {
  ...
  if (!user_id) return res.status(404).json({ error: 'Please provide user_id' })
...
router.patch('/users', async (req, res) => {
  ...
  if (!(user_id && password)) return res.status(400).json({ error: 'Please provide user_id' })
  ...
  else res.status(400).json({ error: "Can't update this user" })
})
