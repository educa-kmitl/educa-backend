...
router.get('/likes', async (req, res) => {
  ...
  if (!(room_id, user_id)) return res.status(400).json({ error: 'Plaese provide room_id and user_id' })
...
router.post('/likes', async (req, res) => {
  ...
  if (!(room_id && user_id)) return res.status(400).json({ error: 'Please provide room_id and user_id' })
