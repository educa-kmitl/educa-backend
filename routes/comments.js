...
router.post('/comments', async (req, res) => {
  ...
  if (!(user_id && resource_id && text && time)) return res.status(400).json({ error: 'Please provide required fields' })
  ...
  try {
    ...
    else res.status(400).json({ error: "Can't add comment" })
  } catch (e) {
    res.status(400).json({ error: e.detail || e.name })
  }
})
