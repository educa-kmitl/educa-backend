...
router.get('/followers', async (req, res) => {
  ...
  if (!user_id) return res.status(400).json({ error: 'Please provide user_id' })
...
router.post('/followings', async (req, res) => {
  ...
  if (!(teacher_id && student_id)) return res.status(400).json({ error: 'Please provide teacher_id and student_id' })
